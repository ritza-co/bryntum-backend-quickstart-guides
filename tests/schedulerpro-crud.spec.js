...
test.describe(`Bryntum Scheduler Pro CRUD Operations [${frontendName} + ${backendName}]`, () => {
    ...
    test('create a new event', async({ page }) => {
        ...
        // Wait for Scheduler Pro to load
        await page.waitForSelector('.b-schedulerpro', { timeout : 5000 });
    ...
    test('edit event name', async({ page }) => {
        ...
        const firstEventUpdated = page.locator('.b-sch-event').first();

        await expect(firstEventUpdated).toContainText('Updated Scheduler Pro Event Name');
    });
